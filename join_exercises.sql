...
LEFT JOIN roles 
	ON roles.id = users.role_id;
-- Right Join-- I expect 5 rows because this one list all the roles even if no one is in the role.
SELECT *
FROM users 
RIGHT JOIN roles 
	ON roles.id = users.role_id;
-- 3. Although not explicitly covered in the lesson, aggregate functions like count can be used with join queries. Use count and the appropriate join type to get a list of roles along with the number of users that has the role. Hint: You will also need to use group by in the query.
SELECT roles.name, COUNT(roles.name) AS 'Number of People in Each Role'
FROM users
RIGHT JOIN roles 
	ON roles.id = users.role_id
GROUP BY roles.name;
...
SELECT dept_name, CONCAT(first_name, ' ', last_name ) AS dept_manager, salaries.salary
FROM departments
JOIN dept_manager 
	ON departments.dept_no = dept_manager.dept_no
JOIN employees 
...
FROM employees
JOIN dept_emp 
	ON dept_emp.emp = employees.emp_no
		AND dept_emp.to_date > curdate()
JOIN departments 
	ON departments.dept_no= dept_emp.dept_no
